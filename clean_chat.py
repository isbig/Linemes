...
def clean_file(file_to_clean):
    ...
    def export_file(df, extend):
        ...
        df.to_csv(export_file_dir, sep='\t', encoding='utf-8')
    ...
    def som(ct):
        ...
        ph = os.getcwd()
        directory = os.path.join(ph, ext)

        file_name = os.path.basename(file_me)
        fn_no_extension = os.path.splitext(file_name)[0]

        # path ที่อยู่ของไฟล์ csv ที่จะเรียกมาซ่อม
        call_csv = os.path.join(directory, fn_no_extension + '_{!s}.csv'.format(ext))

        # เปิดไฟล์
        last_csv = pd.read_csv(call_csv, sep='\t')

        # แปลงแต่ละคอลัมน์เป็นลิสต์
        word_count_set = last_csv['wordCount'].tolist()
        sen_count_set = last_csv['senCount'].tolist()
        sub_count_set = last_csv['sAppear'].tolist()
        who_set = last_csv['menWho'].tolist()
        toq_set = last_csv['senType'].tolist()
        say_set = last_csv['doType'].tolist()
        ...
        for v in list(type_set.values()):
            for n, i in enumerate(v):
                while i == str({}):
                    ...
                    if v[n] == {}:
                        print('ทำอีกรอบ พิจารณา ประโยค : ' + text[n])
                        sleep(20)
                    else:
                        i = v[n]
